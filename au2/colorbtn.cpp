...
/*    COLORREF prevColor = DC->SetTextColor(TextColor);
	...
	COLORREF prevColor = DC->SetTextColor(TextColor);
	DC->SetBkMode(TRANSPARENT);
	// jt Draw the button caption in either single line or multiple lines. 
	// Works only with Win95, NT 4.0 or later
	//DC->DrawText(Buf, strlen(Buf), R, DT_CENTER|DT_VCENTER|DT_SINGLELINE);
	long style = GetWindowLong(m_hWnd, GWL_STYLE);
	if (style |= BS_MULTILINE)
	{
		  CRect R1(R);
		  DC->DrawText(Buf, strlen(Buf), R1, DT_CALCRECT | DT_WORDBREAK);
		  SIZE size;
		  size.cx = (R.Width() - R1.Width()) / 2;
		  size.cy = (R.Height() - R1.Height()) / 2;
		  R += size;

		  DC->DrawText(Buf, strlen(Buf), R, DT_WORDBREAK);
	}
	else
		  DC->DrawText(Buf, strlen(Buf), R, DT_CENTER|DT_VCENTER|DT_SINGLELINE);
